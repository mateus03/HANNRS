...
namespace gazebo{
  ...
  class ManelControl : public ModelPlugin{
	...
	this->updateConnection = event::Events::ConnectWorldUpdateBegin(
	    boost::bind(&ManelControl::UpdateChild, this));
	
	cmd_sub = nh.subscribe("/cmd_vel_Manel",1, &ManelControl::cmdCallBack, this);
	people_sub = nh.subscribe("hannrs/people", 1, &ManelControl::peopleCallBack, this);
	...
	fst_cmd = false;
	  ...
	  for(unsigned int i = 0; i < people_list.people.size(); i++){
	    ...
	    if(person.name.compare("Manel") == 0){
	      pose = person.pose;
	    }
	  }
      }
  ...
  GZ_REGISTER_MODEL_PLUGIN(ManelControl);
}
